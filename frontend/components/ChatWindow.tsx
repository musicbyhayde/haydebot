...
export default function ChatWindow({ item, messages, onSend, onBack }: ChatWindowProps) {
    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-[#EBEAE5] relative" dir="rtl">
            ...
            <div className="bg-white backdrop-blur-xl px-4 md:px-6 py-4 border-b border-slate-200 flex justify-between items-center z-10 sticky top-0">
                ...
                {Object.keys(groupedMessages).map((date) => (
                    <div key={date}>
                        <div className="flex justify-center mb-6">
                            <span className="bg-white backdrop-blur-sm text-slate-500 text-[11px] font-bold px-4 py-1.5 rounded-full shadow-sm">
                                {date}
                            </span>
                        </div>
