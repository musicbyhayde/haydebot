...
export default function LeadDetailPanel({ lead, currentUserName, isAdmin = false, onClose, onStatusChange }: LeadDetailPanelProps) {
    ...
    const [introCustomName, setIntroCustomName] = useState('');
    ...
    useEffect(() => {
        ...
        setIntroCustomName(lead.fields.Name || '');
        setEditData({
    ...
    const fetchVideos = async () => {
        try {
            ...
            setVideoOptions(data.filter(v => v.fields.Is_Active));
        } catch (e) {
            console.error(e);
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-stretch md:items-center justify-end bg-black/30 backdrop-blur-sm" dir="rtl" onClick={onClose}>
            <div className="w-full md:w-[560px] h-full md:h-[90vh] bg-white md:rounded-r-2xl shadow-2xl flex flex-col relative" onClick={(e) => e.stopPropagation()}>
                ...
                {isIntroModalOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm" onClick={() => setIsIntroModalOpen(false)}>
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200" dir="rtl" onClick={(e) => e.stopPropagation()}>
                            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-green-50">
                                ...
                                <button onClick={() => setIsIntroModalOpen(false)} className="text-slate-400 hover:text-slate-600">
                                    <X className="w-6 h-6" />
                                </button>
                            </div>
